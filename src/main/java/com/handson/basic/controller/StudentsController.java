...
public class StudentsController {
    ...
    @RequestMapping(value = "/highGraduationScore", method = RequestMethod.GET)
    public ResponseEntity<?> getHighGraduationScoreStudents(@RequestParam Double graduationScore){
        return new ResponseEntity<>(studentService.getStudentWithGraduationScoreGreaterThan(graduationScore), HttpStatus.OK);
    }

    @RequestMapping(value = "/outstanding", method = RequestMethod.GET)
    public ResponseEntity<?> getOutstandingStudents(){
        return new ResponseEntity<>(studentService.getOutstandingStudents(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> uploadStudentImage(@PathVariable Long id,  @RequestParam("image") MultipartFile image) {
        Optional<Student> dbStudent = studentService.findById(id);
        if (dbStudent.isEmpty()) throw new HandsonException("Student with id: " + id + " not found");
        String bucketPath = "apps/roeis/student-" +  id + ".png";
